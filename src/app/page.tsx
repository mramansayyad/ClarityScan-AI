...
export default function HomePage() {
  ...
  const handleFileUpload = useCallback(
    async (file: File) => {
      ...
      try {
        let documentContent = '';
        const supportedTextBasedFiles = ['text/plain', 'text/csv', 'application/json', 'text/markdown'];
        ...
        if (!documentContent.trim()) {
          throw new Error('Could not extract any text from the document. The file might be empty or unreadable.');
        }
    ...
    <>
        {isLoading ? (
            <div className="flex flex-col items-center justify-center h-full min-h-[calc(100vh-200px)] gap-4 text-center">
              ...
              <p className="text-muted-foreground max-w-md">Our AI is working its magic. This may take a moment.</p>
            </div>
          ) : analysis ? (
            <div className="flex flex-col gap-6">
                <div className="flex flex-col sm:flex-row items-start sm:items-center sm:justify-between gap-4">
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                        <Button variant="outline" onClick={() => handleDownload('pdf')}><Download /> PDF</Button>
                        <Button variant="outline" onClick={() => handleDownload('csv')}><FileType /> CSV</Button>
                        <Button variant="outline" onClick={() => handleDownload('json')}><FileJson /> JSON</Button>
